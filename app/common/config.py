...
# # Define RabbitMQ connection parameters for pika
...
#             temperature=temperature
#             # NOTE: max_tokens is not currently supported in ChatGoogleGenerativeAI
#         )
...
# QUEUE_URL_TASKS = "url_tasks"                  # All processing tasks (new or retried)
# QUEUE_EVALUATION_TASKS = "evaluation_tasks"    # Tasks for evaluator (from processor output)
# QUEUE_STATUS_UPDATES = "status_updates"        # Updates for client

# # -------------------------------
# # Allowed Domains for URL Submission
# # -------------------------------
# # URLs submitted to the system must come from one of these domains.
# # This list is used for initial validation in the FastAPI server (main.py).
# ALLOWED_DOMAINS = {
#     "www.bbc.com",
#     "edition.cnn.com",
#     "www.reuters.com",
...
# # -------------------------------
# # These settings control the behavior of the Processor and Evaluator workers.

# # Prefetch count for RabbitMQ consumers:
# # A value of 1 ensures messages are processed one by one.
# PROCESSOR_PREFETCH_COUNT = int(os.getenv("PROCESSOR_PREFETCH_COUNT", 1))
# EVALUATOR_PREFETCH_COUNT = int(os.getenv("EVALUATOR_PREFETCH_COUNT", 1))
# WEBSOCKET_CONSUMER_PREFETCH_COUNT = int(os.getenv("WEBSOCKET_CONSUMER_PREFETCH_COUNT", 10))

# # Score threshold for the Evaluator:
# # URLs with an overall_score below this threshold will be re-queued for reprocessing.
# EVALUATOR_SCORE_THRESHOLD = float(os.getenv("EVALUATOR_SCORE_THRESHOLD", 7.0))

# # Maximum number of retries for tasks:
# # If a task fails this many times, it will be discarded (nacked without requeue).
# MAX_RETRIES_URL_TASK = int(os.getenv("MAX_RETRIES_URL_TASK", 3))
# MAX_RETRIES_EVALUATION_TASK = int(os.getenv("MAX_RETRIES_EVALUATION_TASK", 3))
